...
void bno055_readAllSensors(bno055_all_data_t *data)
{
    uint8_t buffer[44];  // 0x08–0x33

    // Read 44 bytes starting from 0x08
    bno055_setPage(0);
    bno055_readData(0x08, buffer, 44);

    // Each value is little-endian signed 16-bit
    int16_t *raw = (int16_t *)buffer;

    // Accelerometer
    data->accel.x = raw[0] / 100.0f;
    data->accel.y = raw[1] / 100.0f;
    data->accel.z = raw[2] / 100.0f;

    // Magnetometer
    data->mag.x = raw[3] / 16.0f;
    data->mag.y = raw[4] / 16.0f;

    // Gyroscope
    data->gyro.x = raw[6] / 16.0f;
    data->gyro.y = raw[7] / 16.0f;
    data->gyro.z = raw[8] / 16.0f;

    // Euler (Heading, Roll, Pitch)
    data->euler.x = raw[9] / 16.0f;
    data->euler.y = raw[10] / 16.0f;
    data->euler.z = raw[11] / 16.0f;

    // Quaternion (W, X, Y, Z)
    data->quat.w = raw[12] / 16384.0f;
    data->quat.x = raw[13] / 16384.0f;
    data->quat.y = raw[14] / 16384.0f;
    data->quat.z = raw[15] / 16384.0f;

    // Linear Acceleration
    data->linear.x = raw[16] / 100.0f;
    data->linear.y = raw[17] / 100.0f;
    data->linear.z = raw[18] / 100.0f;

    // Gravity Vector
    data->gravity.x = raw[19] / 100.0f;
    data->gravity.y = raw[20] / 100.0f;
    data->gravity.z = raw[21] / 100.0f;
}
